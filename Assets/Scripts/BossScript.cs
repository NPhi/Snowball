...
public class BossScript : MonoBehaviour
{
		...
		else
		{
			...
			else
			{
				...
				if (positionTarget == Vector2.zero)
				{
					...
					Vector2 randomPoint = new Vector2( Random.Range(0.6f, 0.9f), Random.Range(0f, 1f));
	...
	private void Spawn()
	{
		...
		{
			weapon.enabled = true;
			weapon.transform.rotation = Quaternion.Euler(0, 0, Random.Range (-30, 70));
			weapon.transform.Rotate(0,0,200*Time.deltaTime);
